            ...
            <div class="panel-body">
               <div class="eventotext">
                  <form action="ajustes.php" method="post" enctype="multipart/form-data">
                     ...
                     <div class="media">
                        <div class="media-left">
                           <img class="media-object" src="<?php echo $row['avatar'];?>" width="70" height="70" />
                        </div>
                        <div class="media-body">
                           <label><?php echo $lang[191]; ?></label>
                           <input style="margin-bottom: 10px;" type="file" name="img" class="form-control" /></input>
                        </div>
                     </div>
                     <br>
                     <?php
                  ...
                  <?php
                     ...
                     $link->query("INSERT INTO images (url,fecha) values ('".strip_tags($url)."','".strip_tags($fecha)."')");
                  ...
                  <?php
                     $query = $link->query('SELECT * FROM usuarios WHERE username = "' .$username. '"');
                              ...
                              
                        {
                          ...
                          ?>
                  <h2 class="h2-reset"><b>Si deseas cambiar el nombre de usuario de tu cuenta, ingresa uno nuevo.</b></h2>
                  <br>
                  <input type="text" id="newusername" name="newusername" maxlength="40" placeholder="<?php echo $_SESSION['username']?>" class="biginputkc">
                  <br><br>
                  <input class="btnkc" name="cambiar" type="submit" value="Cambiar nombre de usuario" style="margin-top: 20px;" />    
               </form>
            </div>
         </div>
               <?php
                  if ($row['username'] && $_POST['newusername'] && !empty($_POST['newusername'])) { 
                    
                  
                     $consulta1 = $link->query("SELECT ID FROM usuarios WHERE username = '".$link->real_escape_string($row['username'])."'");
                     
                     $resultados = mysqli_fetch_row($consulta1);
                     $user_id = $resultados[0] ? true : false;
                     ...
                     $username = $resultados2[0] ? true : false;
                     ...
                     $link->query("UPDATE usuarios SET username='".$link->real_escape_string(strip_tags($_POST['newusername']))."' WHERE ID=".$row['ID'].";");
                     echo '<script>location = "kernel/login/cerrar.php";</script>';
                  ...
                  <?php
                     $query = $link->query('SELECT * FROM usuarios WHERE username = "' .$username. '"');
                              ...
                              
                        {
                          ...
                          ?>
                  <h2 class="h2-reset"><b>Si deseas cambiar la contraseña de tu cuenta, ingresa una nueva aqui.</b></h2>
                  <br>
                  <input type="text" id="password" name="password" maxlength="40" placeholder="Nueva contraseña" class="biginputkc">
                  <br><br>
                  <input class="btnkc" name="cambiar" type="submit" value="Cambiar contraseña" style="margin-top: 20px;" />    
               </form>
            </div>
         </div>
         <?php
            if ($_SESSION['username'] && $_POST['password'] && !empty($_POST['password'])) {
                    
             $consulta1 = $link->query("SELECT username FROM usuarios WHERE username = '".$_SESSION['username']."'");
             $resultados = mysqli_fetch_row($consulta1);
              $user_id = $resultados[0] ? true : false;
               ...
               if (!$user_id) {
                ...
                echo "<br><div class='alerta-no'>$lang[383]</div>";
                       ...
                       $link->query("UPDATE usuarios SET password='".strip_tags($password)."' WHERE username='".$_SESSION['username']."'");//Doble cifrado igual a registro, mira la variable cifrado1 no esta o si como en lo de inserta
